(ns worldspan.main
  "Kotodama WASM entrypoint for the Worldspan clean-room actor (L4) — Clojure port.
  ...
  facts (`worldspan.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [kotoba.lang.text :as str]))

(def ns-prefix "worldspan")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"worldspan.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "worldspan-compat" :tier tier :entities entities} 200])
...
(defn start! [] :worldspan-compat/ready)
